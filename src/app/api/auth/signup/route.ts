...
export async function POST(request: Request) {
  try {
    ...
    // ─────────────────────────────
    // パスワードのハッシュ化
    // ・ユーザーのパスワードを安全に保管するため、bcryptを使ってハッシュ化する
    // ─────────────────────────────
    const hashedPassword = await bcrypt.hash(password, 10);

    // ─────────────────────────────
    // 新規ユーザーの作成
    // ・Prismaを利用して、ユーザー情報をデータベースに保存する
    // ─────────────────────────────
    ...
    // ─────────────────────────────
    // レスポンス用ユーザーオブジェクトの整形
    // ・セキュリティのため、ハッシュ化済みパスワードはレスポンスから除外する
    // ─────────────────────────────
